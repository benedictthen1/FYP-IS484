...
app.layout = html.Div([
    ...
    dbc.Row([
        dbc.Col(dbc.Table(id="equities_companies_table_rows", striped=True, bordered=True, hover=True, size='sm'),width=4),
        dbc.Col(dbc.Table(id="fixed_income_companies_table_rows", striped=True, bordered=True, hover=True, size='sm'),width=4),
        dbc.Col(dbc.Table(id="alternatives_clients_table_rows", striped=True, bordered=True, hover=True, size='sm'),width=4),
    ])
...
@app.callback(
    ...
    Output('equities_companies_table_rows','children'),
    Output('fixed_income_companies_table_rows','children'),
    Output('alternatives_clients_table_rows','children')],
...
def return_thresholds_set_table_and_affected_companies_clients(n_intervals):
    if n_intervals is None:
        ...
        if not is_thresholds_set():
            ...
            fixed_income_companies_table_rows = []
            alternatives_clients_table_rows = []
        else:
            set_values_df = pd.read_csv('set_values_df.csv')
            ...
            equities_companies_table_rows = [html.Tr(html.Td(company)) for company in equities_companies_list]
            fixed_income_companies_table_rows = [html.Tr(html.Td(company)) for company in fixed_income_companies_list]
            alternatives_clients_table_rows = [html.Tr(html.Td(client)) for client in alternatives_clients_list]

    return output_msg, set_values_table,\
    equities_companies_no, fixed_income_companies_no, alternatives_clients_no,\
    equities_companies_table_rows, fixed_income_companies_table_rows, alternatives_clients_table_rows
